public class VPNHelp : NSObject {
  ...
  public class func showHelp() -> Int32 {
    ...
    DDLogInfo("Examples:")
    DDLogInfo("")
    DDLogInfo(" \033[2m Creating a single L2TP over IPSec VPN Service ")
    ...
    DDLogInfo("")
    DDLogInfo("  \033[2mNote: for Cisco IPSec you would use --cisco \033[2minstead of\033[0m --l2tp\033[2m and\033[0m -c\033[2m instead of\033[0m -l\033[2m")
    ...
    DDLogInfo("")
    DDLogInfo("  \033[2mNote: for L2TP, you can add the --split \033[2mflat to *not* force all traffic over VPN.")
    DDLogInfo("")
    DDLogInfo(" \033[2m The same command but shorter ")
    DDLogInfo("  sudo macosvpn create l2tp Atlantic endpoint atlantic.example.com username Alice password p4ssw0rd shared-secret s3same")
    DDLogInfo("")
    DDLogInfo(" \033[2m The same command even shorter ")
    DDLogInfo("  sudo macosvpn create -l Atlantic -e atlantic.example.com -u Alice -p p4ssw0rd -s s3same")
    DDLogInfo("")
    DDLogInfo(" \033[2m The same command as short as possible ")
    DDLogInfo("  sudo macosvpn create -leups Atlantic atlantic.example.com Alice p4ssw0rd s3same ")
    DDLogInfo("")
    DDLogInfo(" \033[2m Repeat the arguments for creating multiple Services at once (no matter which short version you use :) ")
    DDLogInfo("  sudo macosvpn create -leups Atlantic atlantic.example.com Alice p4ssw0rd s3same \\")
    DDLogInfo("                       -leups Northpole northpole.example.com Bob s3cret pr1v4te")
    DDLogInfo("")
    DDLogInfo(" \033[2m Assign default values which will be applied to every service ")
    ...
    DDLogInfo("")
    DDLogInfo(" \033[2m This application is released under the MIT license. ")
  ...
  private class func copyrightNotice() -> String {
    return " \033[2m Copyright (c) 2014-\(self.currentYear()) halo. See https://github.com/halo/macosvpn "
  }
